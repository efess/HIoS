...
typedef void(*bufferCallback)(char*, int16_t);
...
public:
	...
	int16_t listAP(bufferCallback cb);
...
private:
	...
	int16_t betterReadNoErr(const char * rsp, uint8_t* bytesRead, unsigned int timeout);
	int16_t betterRead(const char * pass, const char * fail, uint8_t* bytesRead, unsigned int timeout);
	...
	char * betterSearchBuffer(const char * test);
